...
public sealed class PdfSharpOfficialDocumentRenderer : IOfficialDocumentRenderer
{
    ...
    public GeneratedDocument Render(OfficialDocument document)
    {
        ...
        pdf.Info.Author = "United States of Mekniy and Lurk";
        pdf.Info.CreationDate = document.IssuedAt.UtcDateTime;

        var context = new RenderContext(pdf, document);
        context.StartPage();
        context.DrawTitle();
        context.DrawFields();
        ...
        return new GeneratedDocument(
            stream.ToArray(),
            "application/pdf",
            $"citizenship-decision-{document.DocumentNumber}.pdf");
    }
    ...
    private sealed class RenderContext
    {
        ...
        public RenderContext(PdfDocument pdf, OfficialDocument document)
        {
            _pdf = pdf;
            _document = document;
        }
        ...
        public void DrawTitle()
        {
            ...
            DrawText(
                $"{_document.IssuedAt:dd MMMM yyyy} · Yeoju, Mekniy",
                _small,
                Muted,
        ...
        {
            EnsureSpace(70);
            DrawText("Application history", _signature, Ink, Margin, _y, ContentWidth);
            _y += 22;
            DrawAuditHeader();
            foreach (var entry in _document.AuditTrail)
                DrawAuditRow(entry);
            _y += 12;
        }
        ...
        private void DrawMasthead()
        {
            ...
            DrawText("MKLU", _title, XColors.White, Margin, 17, 80);
            DrawText("UNITED STATES OF MEKNIY AND LURK", _smallBold, XColors.White, Margin + 86, 18, 250);
            DrawText("OFFICIAL DECISION", _small, XColor.FromArgb(190, 190, 190), Margin + 86, 33, 250);
        }
        ...
        private void DrawAuditRow(OfficialDocumentAuditEntry entry)
        {
            ...
            {
                StartPage();
                DrawText("Application history (continued)", _signature, Ink, Margin, _y, ContentWidth);
                _y += 22;
                DrawAuditHeader();
            }
